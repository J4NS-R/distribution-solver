...
def main():
    prob = simpleprob.SimpleProblem("maps/map_%d.input" % MAPNO)

    best_cost = shuffle_runner.solve(prob, 0)  # to start with
    ...
    while True:
        prob.clear()
        cost = shuffle_runner.solve(prob, x, max_cost=best_cost)
